...
describe('authController', () => {
    ...
    describe('register', () => {
        it('should return 400 if missing fields', async () => {
            ...
            expect(res.json).toHaveBeenCalledWith({ error: { message: 'Email, username, and password are required' } });
        });

        it('should return 409 if user already exists', async () => {
            req.body = { email: 'test@test.com', username: 'testuser', password: 'password123' };
            userModel.findUserByEmailOrUsername.mockResolvedValue({ email: 'test@test.com' });
            
            await register(req, res, next);
            
            expect(res.status).toHaveBeenCalledWith(409);
            expect(res.json).toHaveBeenCalledWith({ error: { message: 'Email is already in use' } });
        });
    ...
    describe('login', () => {
        ...
        it('should return 401 if user not found', async () => {
            ...
            expect(res.json).toHaveBeenCalledWith({ error: { message: 'Invalid credentials' } });
        });

        it('should return 401 if password does not match', async () => {
            req.body = { identifier: 'testuser', password: 'password123' };
            userModel.findUserByEmailOrUsername.mockResolvedValue({ password_hash: 'hash' });
        ...
        it('should login successfully and return tokens', async () => {
            req.body = { identifier: 'testuser', password: 'password123' };
            userModel.findUserByEmailOrUsername.mockResolvedValue({ id: '123', username: 'testuser', email: 'test@test.com', password_hash: 'hash' });
    ...
    describe('refreshToken', () => {
        ...
        it('should return 401 if token is expired', async () => {
            ...
            userModel.findRefreshToken.mockResolvedValue({ expires_at: new Date(Date.now() - 10000) });
            userModel.deleteRefreshToken.mockResolvedValue(true);

            await refreshToken(req, res, next);

            expect(res.status).toHaveBeenCalledWith(401);
            expect(userModel.deleteRefreshToken).toHaveBeenCalledWith('expired-token');
        });

        it('should return 401 if token signature is invalid', async () => {
            req.body = { refreshToken: 'bad-sig-token' };
            userModel.findRefreshToken.mockResolvedValue({ expires_at: new Date(Date.now() + 10000) });
            jwtUtils.verifyToken.mockImplementation(() => { throw new Error('Invalid signature'); });
        ...
        it('should refresh token successfully', async () => {
            ...
            userModel.findRefreshToken.mockResolvedValue({ expires_at: new Date(Date.now() + 10000) });
            jwtUtils.verifyToken.mockReturnValue({ userId: '123' });
            userModel.findUserById.mockResolvedValue({ id: '123', username: 'testuser' });
